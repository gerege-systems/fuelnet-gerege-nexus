 ...
 * fuel_stations carries a tenant_id. A list scraped from OpenStreetMap is
 ...
 * actually describes:
 *
 *	go run ./cmd/fuel-import -tenant demo -file stations.json
 *	go run ./cmd/fuel-import -tenant petrovis -file stations.json -brand petrovis
 *
 * # Row-level security
 *
...
func run(tenantSlug, file, brand string, dryRun, demoStock bool) error {
	...
	err = pool.QueryRow(ctx,
		`SELECT id::text FROM platform.tenants WHERE slug = $1`, tenantSlug).Scan(&tenantID)
	if err != nil {
	...
	for _, s := range selected {
		var id string
		err := tx.QueryRow(ctx, `
			INSERT INTO tenant.fuel_stations
		...
		for code, fuel := range fuels {
			...
			_, err := tx.Exec(ctx, `
				INSERT INTO tenant.fuel_station_inventory
				...
				DO UPDATE SET fuel_label = EXCLUDED.fuel_label,
				              ...
				              tank_capacity_liters = CASE
				                  ...
				                  ELSE tenant.fuel_station_inventory.tank_capacity_liters END,
				              current_stock_liters = CASE
				                  WHEN EXCLUDED.tank_capacity_liters > 0
				                  THEN EXCLUDED.current_stock_liters
				                  ELSE tenant.fuel_station_inventory.current_stock_liters END`,
