...
func TestRecordPersistsToDatabase(t *testing.T) {
	...
	_, err := pool.Exec(ctx, `DELETE FROM tenant.audit_events WHERE action = $1`, action)
	if err != nil {
		t.Fatalf("clean up before: %v", err)
	}
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(), `DELETE FROM tenant.audit_events WHERE action = $1`, action)
	})
	...
	err = pool.QueryRow(ctx,
		`SELECT count(*) OVER (), resource, user_id, details->>'reason'
		   FROM tenant.audit_events WHERE action = $1`, action).
...
func TestRecordAcceptsNonUUIDActor(t *testing.T) {
	...
	if err := pool.QueryRow(ctx,
		`SELECT user_id FROM tenant.audit_events WHERE action = $1`, action).Scan(&stored); err != nil {
		t.Fatalf("read back: %v", err)
	}
