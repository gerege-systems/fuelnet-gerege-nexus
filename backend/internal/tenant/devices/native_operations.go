...
func (h *Handlers) HandleDeviceTelemetry(w http.ResponseWriter, r *http.Request) {
	...
	for _, event := range req.Events {
		...
		if _, err = tx.Exec(r.Context(), `INSERT INTO tenant.device_telemetry(tenant_id,device_id,level,event,payload,occurred_at) VALUES($1,$2,$3,$4,$5,$6)`, device.TenantID, device.ID, level, event.Event, raw, event.OccurredAt); err != nil {
			httpx.Error(w, 503, "telemetry unavailable")
			return
		}
	}
...
func (h *Handlers) HandleRegisterPushToken(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.db.Exec(r.Context(), `INSERT INTO tenant.push_tokens(tenant_id,user_id,provider,token_hash,token_ciphertext,app_id) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(token_hash) DO UPDATE SET user_id=EXCLUDED.user_id,tenant_id=EXCLUDED.tenant_id,provider=EXCLUDED.provider,token_ciphertext=EXCLUDED.token_ciphertext,app_id=EXCLUDED.app_id,updated_at=NOW()`, claims.TenantID, claims.UserID, req.Provider, caseSensitiveSecretHash(req.Token), encrypted, req.AppID)
	if err != nil {
