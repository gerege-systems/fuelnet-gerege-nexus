...
func lockoutUser(t *testing.T, pool *pgxpool.Pool) string {
	...
	if err := pool.QueryRow(ctx,
		`INSERT INTO users(email, password_hash, name) VALUES($1, 'x', 'lockout probe') RETURNING id::text`,
...
func lockoutState(t *testing.T, pool *pgxpool.Pool, userID string) (attempts int, locked bool) {
	...
	if err := pool.QueryRow(context.Background(),
		`SELECT failed_login_attempts, COALESCE(locked_until > NOW(), FALSE) FROM users WHERE id=$1`,
...
func TestLapsedLockoutDoesNotRelockOnASingleFailure(t *testing.T) {
	...
	if _, err := pool.Exec(context.Background(),
		`UPDATE users SET failed_login_attempts=$2, locked_until=NOW() - INTERVAL '1 second' WHERE id=$1`,
