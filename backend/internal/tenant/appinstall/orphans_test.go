...
func TestAnInstalledAppThisBinaryCannotServeIsReported(t *testing.T) {
	...
	if _, err := pool.Exec(ctx,
		`INSERT INTO tenants (id, name, slug) VALUES ($1, 'Uncarried', $2)`,
	...
	if _, err := pool.Exec(ctx,
		`INSERT INTO apps (id, slug, name, description, icon_url, category)
		 VALUES ($1, 'departed', 'Departed', '', '', 'Test') ON CONFLICT (id) DO NOTHING`,
		departed); err != nil {
		t.Fatalf("app row: %v", err)
	}
	t.Cleanup(func() { _, _ = pool.Exec(context.Background(), `DELETE FROM apps WHERE id = $1`, departed) })
	if _, err := pool.Exec(ctx,
