	...
	rows, err := m.db.Query(r.Context(), `
		SELECT d.id::text, d.name, d.brand, d.aimag, d.district, d.address,
		       ...
		       COALESCE(
		           ...
		           '[]'::json)
		  FROM fuel_depots d
		  LEFT JOIN fuel_depot_tanks t ON t.depot_id = d.id
		 GROUP BY d.id
...
func (m *Module) handleCreateDepot(w http.ResponseWriter, r *http.Request) {
	...
	nexus.Audit(r.Context(), tenantID, claims.UserID, "fuel.depot.created", depot.ID,
...
func (m *Module) handleCreateTank(w http.ResponseWriter, r *http.Request) {
	...
	var exists bool
	err = m.db.QueryRow(r.Context(),
		`SELECT true FROM fuel_depots WHERE id = $1::uuid`, depotID).Scan(&exists)
	...
	nexus.Audit(r.Context(), tenantID, claims.UserID, "fuel.tank.created", tank.ID,
...
func (m *Module) handleReceiveIntoDepot(w http.ResponseWriter, r *http.Request) {
	...
	err = tx.QueryRow(r.Context(), `
		SELECT fuel_type, depot_id::text FROM fuel_depot_tanks
	...
	if request.ShipmentID != "" {
		...
		err = tx.QueryRow(r.Context(), `
			SELECT s.status, s.fuel_type,
			       (SELECT b.id::text FROM fuel_batches b WHERE b.customs_shipment_id = s.id LIMIT 1)
			  FROM fuel_customs_shipments s
	...
	if batchID != nil {
		if err := tx.QueryRow(r.Context(),
			`SELECT batch_code FROM fuel_batches WHERE id = $1::uuid`, *batchID).
	...
	nexus.Audit(r.Context(), tenantID, claims.UserID, "fuel.depot.received", receipt.ID,
...
func (m *Module) handleListDepotReceipts(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := m.db.Query(r.Context(), `
		SELECT rc.id::text, rc.depot_id::text, rc.tank_id::text,
		  ...
		  LEFT JOIN fuel_batches b ON b.id = rc.batch_id
