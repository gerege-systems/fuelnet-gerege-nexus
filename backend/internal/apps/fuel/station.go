 ...
 *
 * Until now the register could only be filled by cmd/fuel-import, which says in
...
func (m *Module) handleCreateStation(w http.ResponseWriter, r *http.Request) {
	...
	nexus.Audit(r.Context(), tenantID, claims.UserID, "fuel.station.created", station.ID,
...
func (m *Module) handleUpdateStation(w http.ResponseWriter, r *http.Request) {
	...
	err = m.db.QueryRow(r.Context(), `
		...
		RETURNING id::text, name, brand, brand_label, lat, lon, aimag, district,
		          ...
		          (SELECT COUNT(*)::int FROM fuel_station_inventory i
		            WHERE i.station_id = fuel_stations.id)`,
	...
	nexus.Audit(r.Context(), tenantID, claims.UserID, "fuel.station.updated", station.ID,
...
// point the other way: fuel_station_receipts cascades from here, so the
...
func (m *Module) handleDeleteStation(w http.ResponseWriter, r *http.Request) {
	...
	if err := m.db.QueryRow(r.Context(), `
		SELECT (SELECT COUNT(*)::int FROM fuel_station_receipts WHERE station_id = $1),
		       (SELECT COUNT(*)::int FROM fuel_dispatch_trips
	...
	tag, err := m.db.Exec(r.Context(), `DELETE FROM fuel_stations WHERE id = $1`, stationID)
	if err != nil {
	...
	nexus.Audit(r.Context(), tenantID, claims.UserID, "fuel.station.deleted", stationID, nil)
...
func (m *Module) handleSetStationGrade(w http.ResponseWriter, r *http.Request) {
	...
	var exists bool
	if err := m.db.QueryRow(r.Context(),
		`SELECT EXISTS (SELECT 1 FROM fuel_stations WHERE id = $1)`, stationID).Scan(&exists); err != nil || !exists {
		nexus.Error(w, http.StatusNotFound, "ШТС олдсонгүй")
		return
	}
	...
	err = m.db.QueryRow(r.Context(), `
		INSERT INTO fuel_station_inventory
		...
		ON CONFLICT (station_id, fuel_type) DO UPDATE SET
		        fuel_label           = COALESCE(NULLIF($4, ''), fuel_station_inventory.fuel_label),
		        price_mnt            = COALESCE($5, fuel_station_inventory.price_mnt),
		        tank_capacity_liters = COALESCE($6, fuel_station_inventory.tank_capacity_liters),
		        status               = COALESCE($7, fuel_station_inventory.status),
		        last_reported_at     = CASE WHEN $7 IS NULL OR $7 = fuel_station_inventory.status
		                                    THEN fuel_station_inventory.last_reported_at
		                                    ELSE NOW() END
	...
	nexus.Audit(r.Context(), tenantID, claims.UserID, "fuel.station.grade_set", stationID,
...
func (m *Module) handleDeleteStationGrade(w http.ResponseWriter, r *http.Request) {
	...
	tag, err := m.db.Exec(r.Context(),
		`DELETE FROM fuel_station_inventory WHERE station_id = $1 AND fuel_type = $2`,
		stationID, fuelType)
	if err != nil {
	...
	nexus.Audit(r.Context(), tenantID, claims.UserID, "fuel.station.grade_removed", stationID,
