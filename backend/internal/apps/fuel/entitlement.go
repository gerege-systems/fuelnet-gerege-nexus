...
func citizen(r *http.Request) (id string, tenantID string, err error) {
	...
	tenant, err := nexus.TenantID(r.Context())
	if err != nil {
		return "", "", err
	}
...
func (m *Module) handleIssueVoucher(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := tx.Exec(r.Context(), `
		UPDATE fuel_vouchers SET status = 'expired'
	...
	err = tx.QueryRow(r.Context(), `
		INSERT INTO fuel_entitlements (citizen_id, tenant_id, for_date, granted_mnt, used_mnt)
		VALUES ($1, $2, CURRENT_DATE, $3, $4)
		ON CONFLICT (citizen_id, for_date) DO UPDATE
		   SET used_mnt = fuel_entitlements.used_mnt + $4,
		       ...
		       granted_mnt = GREATEST(fuel_entitlements.granted_mnt, EXCLUDED.granted_mnt),
		       updated_at = NOW()
		 WHERE fuel_entitlements.used_mnt + $4 <= fuel_entitlements.granted_mnt
	...
	err = tx.QueryRow(r.Context(), `
		...
		SELECT i.id::text, i.amount_mnt::float8, i.fuel_type, i.fuel_label,
		  ...
		  FROM inserted i
		  LEFT JOIN fuel_stations s ON s.id = i.intended_station_id`,
	...
	nexus.Audit(r.Context(), tenantID, citizenID, "fuel.voucher.issued", voucher.ID,
...
func (m *Module) handleMyVouchers(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := m.db.Query(r.Context(), `
		SELECT v.id::text, v.amount_mnt::float8, v.fuel_type, v.fuel_label,
		  ...
		  LEFT JOIN fuel_stations s ON s.id = v.intended_station_id
