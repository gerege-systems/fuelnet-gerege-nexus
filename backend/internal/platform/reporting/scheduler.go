...
func (s *Scheduler) run(ctx context.Context, schedule dueSchedule) {
	...
	if _, err := s.engine.DB().Exec(tenant.Without(context.WithoutCancel(ctx)),
...
func (s *Scheduler) produceAndDeliver(ctx context.Context, schedule dueSchedule) error {
	...
	audit.Record(tenant.WithTenantID(ctx, schedule.TenantID), schedule.TenantID, schedule.CreatedBy,
