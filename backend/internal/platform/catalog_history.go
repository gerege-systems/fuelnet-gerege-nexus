...
func (s *Server) appReleaseHistory(r *http.Request, appID, locale string) ([]historyEntry, error) {
...
func (s *Server) appInstallationHistory(r *http.Request, tenantID, appID string) ([]historyEntry, string, error) {
	rows, err := s.db.Query(r.Context(),
		`SELECT e.event_type, e.details, e.created_at, ai.installed_version,
		        COALESCE(u.name, '')
		   FROM installation_events e
		   JOIN app_installations ai ON ai.id = e.installation_id
		   LEFT JOIN users u
