...
func googleServer(t *testing.T, mutate ...func(*ssoclient.Config)) *Server {
	...
	return &Server{googleLogin: ssoclient.New(cfg)}
}

func ssoConfigOf(t *testing.T, server *Server) map[string]any {
	t.Helper()
...
func TestGoogleIsOffUntilItIsConfigured(t *testing.T) {
	...
	for name, handler := range map[string]http.HandlerFunc{
		"start":    (&Server{}).handleGoogleStart,
		"callback": (&Server{}).handleGoogleCallback,
	} {
		rec := httptest.NewRecorder()
