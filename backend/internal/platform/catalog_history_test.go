...
func installedApp(t *testing.T, pool *pgxpool.Pool) (tenantID, userID, appID, installID string) {
	...
	if err := pool.QueryRow(ctx,
		`INSERT INTO users (email, password_hash, name) VALUES ($1,'x',$2) RETURNING id::text`,
	...
	if _, err := pool.Exec(ctx,
		`INSERT INTO apps (id, slug, name) VALUES ($1, $2, $3)`,
	...
	if err := pool.QueryRow(ctx,
		`INSERT INTO app_installations (tenant_id, app_id, installed_version)
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(bg, `DELETE FROM apps WHERE id=$1`, appID)
...
func event(t *testing.T, pool *pgxpool.Pool, installID, kind string, details map[string]string) {
	...
	if _, err := pool.Exec(context.Background(),
		`INSERT INTO installation_events (installation_id, event_type, details, created_at)
