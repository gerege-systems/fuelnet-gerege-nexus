...
func CreateOperator(ctx context.Context, db *pgxpool.Pool, params NewOperator) (Operator, Enrolment, error) {
	...
	if err := tx.QueryRow(ctx,
		`SELECT EXISTS (SELECT 1 FROM operator_accounts WHERE lower(email) = $1)`, email).
		Scan(&taken); err != nil {
...
func ConfirmSecondFactor(ctx context.Context, db *pgxpool.Pool, operatorID, code string) error {
	...
	var confirmed bool
	err := db.QueryRow(ctx,
		`SELECT totp_secret, totp_confirmed_at IS NOT NULL FROM operator_accounts WHERE id = $1::uuid`,
