...
func (s *Service) Do(ctx context.Context, sess Session, change Change, fn func(context.Context, pgx.Tx) error) error {
	if change.Reason == "" {
		return ErrReasonRequired
	}
	return s.do(ctx, sess, change, fn)
}

// do is Do without the reason requirement, for the writes that are their own
// reason: signing in, signing out, confirming a second factor. Asking an
...
// one target.
func (s *Service) ListAudit(ctx context.Context, action, targetType, targetID string) ([]AuditEntry, error) {
	rows, err := s.db.Query(scoped(ctx),
		`SELECT id::text, operator_id::text, operator_email, action, target_type, target_id,
		  ...
		  LIMIT $4`,
		action, targetType, targetID, auditPageSize)
	if err != nil {
