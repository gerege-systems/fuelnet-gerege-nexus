...
func TestRuntimeCollectorsAreRegistered(t *testing.T) {
	if !observability.RuntimeCollectorsRegistered() {
...
func TestPoolCollectorWithoutPool(t *testing.T) {
	...
	if err := registry.Register(observability.NewPoolCollector(nil)); err != nil {
...
func TestObserveExternalRecordsOutcome(t *testing.T) {
	before := histogramCount(t, "external_request_duration_seconds", map[string]string{
		"system": observability.SystemEID, "operation": "unit_test", "status": "error",
	})
...
func TestUnknownExternalSystemIsFoldedIntoOther(t *testing.T) {
	if got := observability.ExternalSystem("some-new-gateway.example.mn"); got != "other" {
		t.Fatalf("expected \"other\", got %q", got)
	}
	if got := observability.ExternalSystem(observability.SystemXYP); got != observability.SystemXYP {
...
func TestBusinessCountersIncrement(t *testing.T) {
	before := counterValue(t, "logins_total", map[string]string{
		"method": observability.LoginPassword, "result": observability.ResultFailure,
	})
...
func TestContextHandlerAddsRequestAndTenant(t *testing.T) {
	...
	logger := slog.New(observability.NewContextHandler(
	...
	ctx = tenant.WithTenantID(ctx, "11111111-1111-1111-1111-111111111111")
