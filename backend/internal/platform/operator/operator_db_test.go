...
func newOperator(t *testing.T, pool *pgxpool.Pool, role Role) (Operator, string) {
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(context.Background(), `DELETE FROM operator_accounts WHERE id = $1::uuid`, operator.ID)
	})
...
func TestSignInRefusesTheWrongPasswordAndLocksOut(t *testing.T) {
	...
	if err := pool.QueryRow(context.Background(),
		`SELECT locked_until IS NOT NULL AND locked_until > NOW() FROM operator_accounts WHERE id = $1::uuid`,
...
func TestOperatorAuditCannotBeRewritten(t *testing.T) {
	...
	if _, err := pool.Exec(ctx,
		`UPDATE operator_audit SET reason = 'rewritten' WHERE operator_id = $1::uuid`,
		operator.ID); err == nil {
		t.Fatal("an audit row was updated")
	}
	if _, err := pool.Exec(ctx,
		`DELETE FROM operator_audit WHERE operator_id = $1::uuid`, operator.ID); err == nil {
