...
export default function AICopilot() {
  ...
  return <>
    <button onClick={()=>setOpen(v=>!v)} aria-label={t("ai.view.title")} className="gerege-copilot-trigger fixed bottom-6 start-6 bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-3.5 rounded-full shadow-lg flex items-center gap-2 z-50"><Sparkles className="w-5 h-5 text-amber-300"/><span className="text-sm font-semibold">{t("ai.view.title")}</span></button>
    {open&&<section className="gerege-copilot-panel fixed bottom-24 start-6 w-[420px] max-w-[calc(100vw-3rem)] bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col z-50 overflow-hidden h-[600px]">
