...
export default function AICopilot() {
  ...
  return <>
    <button onClick={()=>setOpen(v=>!v)} aria-label={t("ai.view.title")} title={t("ai.view.title")} aria-expanded={open} aria-controls="gerege-copilot"
      className={`gerege-copilot-trigger w-10 h-10 rounded-full grid place-items-center border transition shrink-0 ${open?"border-indigo-200 bg-indigo-50 text-indigo-700":"border-slate-200 bg-white text-indigo-600 hover:bg-slate-50"}`}><Sparkles className="w-5 h-5"/></button>
    ...
    <section id="gerege-copilot" aria-hidden={!open} aria-label={t("ai.view.subtitle")}
      className={`gerege-copilot-drawer bg-white border-s border-slate-200 shadow-2xl flex flex-col ${open?"is-open":""}`}>
      <header className="bg-gradient-to-r from-slate-900 to-indigo-950 p-4 text-white flex items-center justify-between shrink-0"><div className="flex items-center gap-2"><Bot className="w-5 h-5 text-indigo-300"/><div><h3 className="font-bold text-sm">{t("ai.view.subtitle")}</h3><p className="text-[11px] text-slate-400">{t("ai.view.engine_note")}</p></div></div><button onClick={()=>setOpen(false)}><X className="w-5 h-5"/></button></header>
