 ...
 * number a citizen's entitlement is spent against — was whatever cmd/fuel-import
 * guessed months ago.
 *
...
import {
  ...
  inputClass,
  litres,
} from "@/components/fuel/operatorUI";

// The grades cmd/fuel-import already writes, spelled the way a Mongolian
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <p className="max-w-2xl text-slate-500">{t("fuel.station.subtitle")}</p>
      ...
      {stations && stations.length > 0 ? (
        <div className="mb-4 flex items-center justify-between gap-4">
          <input
            ...
            placeholder={t("fuel.station.search")}
            value={query}
            onChange={(event) => setQuery(event.target.value)}
          />
          <span className="text-sm text-slate-500">
            {t("fuel.view.count")}: <b className="text-slate-900">{shown?.length ?? 0}</b>
          </span>
        </div>
      ) : null}
...
function StationCard({
  ...
  const remove = async () => {
    if (!window.confirm(t("fuel.station.delete_confirm"))) return;
    try {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5">
      <header className="mb-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-500">
            ...
            <span>
              {t("fuel.station.pumps")}: {station.active_pumps}/{station.total_pumps}
            </span>
      ...
      {grades.length === 0 && !addingGrade ? (
        <p className="text-sm text-slate-400">{t("fuel.station.no_grades")}</p>
      ) : (
      ...
      <p className="mt-3 text-xs text-slate-400">{t("fuel.station.stock_note")}</p>
    </section>
  );
}
  ...
  return (
    <form onSubmit={submit} className="mb-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Field label={t("fuel.station.price")}>
          <input
            type="number"
            min="0"
            className={inputClass}
        ...
        <Field label={t("fuel.station.capacity")}>
          <input
            type="number"
            min="0"
            className={inputClass}
        ...
        <Field label={t("fuel.station.status")}>
          <select
            className={inputClass}
            value={form.status}
  ...
  return (
    <form onSubmit={submit} className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Field label={t("fuel.station.name")}>
          <input
            required
            className={inputClass}
            value={form.name}
        ...
        <Field label={t("fuel.station.brand")}>
          <input
            className={inputClass}
        ...
        <Field label={t("fuel.station.phone")}>
          <input
            className={inputClass}
            value={form.phone}
        ...
        <Field label={t("fuel.station.aimag")}>
          <input
            className={inputClass}
            value={form.aimag}
        ...
        <Field label={t("fuel.station.district")}>
          <input
            className={inputClass}
            value={form.district}
        ...
        <Field label={t("fuel.station.address")}>
          <input
            className={inputClass}
            value={form.address}
        ...
        <Field label={t("fuel.station.lat")} hint={t("fuel.station.coords_hint")}>
          <input
            required
            type="number"
            step="any"
        ...
        <Field label={t("fuel.station.lon")}>
          <input
            required
            type="number"
            step="any"
        ...
        <Field label={t("fuel.station.hours")}>
          <input
            className={inputClass}
        ...
        <Field label={t("fuel.station.pumps")}>
          <input
            type="number"
            min="0"
            className={inputClass}
        ...
        <Field label={t("fuel.station.active_pumps")}>
          <input
            type="number"
            min="0"
            className={inputClass}
        ...
        {station ? (
          <Field label={t("fuel.station.status")}>
            <select
              ...
              onChange={(event) => setForm({ ...form, status: event.target.value })}
            >
              <option value="available">{t("fuel.station.status.available")}</option>
              <option value="closed">{t("fuel.station.status.closed")}</option>
            </select>
          </Field>
        ) : null}
      </div>
