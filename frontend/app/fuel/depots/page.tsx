  ...
  return (
    <div>
      <div className="mb-6 flex items-start justify-between gap-6">
        <p className="max-w-2xl text-slate-500">{t("fuel.depot.subtitle")}</p>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5">
      <header className="mb-4 flex flex-wrap items-start justify-between gap-4">
          ...
          <h2 className="flex items-center gap-2 font-semibold text-slate-900">
            ...
            {depot.has_rail_siding ? (
              <span
                ...
                title={t("fuel.depot.rail")}
              >
                <Train className="h-3 w-3" />
                {depot.rail_station_code || t("fuel.depot.rail")}
              </span>
            ) : null}
          </h2>
        ...
        <div className="flex items-center gap-2">
          <GhostButton onClick={onAddTank}>{t("fuel.depot.add_tank")}</GhostButton>
          <PrimaryButton onClick={onReceive} disabled={!canReceive}>
            <Droplets className="h-4 w-4" />
            {t("fuel.depot.receive")}
          </PrimaryButton>
        </div>
      </header>

      {tanks.length === 0 ? (
        <p className="text-sm text-slate-400">{t("fuel.depot.no_tanks")}</p>
      ) : (
  ...
  return (
    <li className="rounded-xl border border-slate-200 p-3">
      ...
      <p className="mt-2 text-xs text-slate-500">
        ...
        {tank.fill_percent.toFixed(1)}% {t("fuel.depot.of_capacity")}
      </p>
  ...
  return (
    <form onSubmit={submit} className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Field label={t("fuel.depot.aimag")}>
          <input
            className={inputClass}
            value={form.aimag}
        ...
        <Field label={t("fuel.depot.rail_code")}>
          <input
            className={inputClass}
...
function TankDialog({
  ...
  return (
    <Dialog title={t("fuel.depot.add_tank")} onCancel={onCancel}>
      <form onSubmit={submit}>
        <div className="grid gap-4">
          <Field label={t("fuel.depot.tank_no")}>
            <input
              required
              className={inputClass}
              value={form.tank_no}
          ...
          <Field label={t("fuel.ship.fuel_type")}>
            <select
              className={inputClass}
          ...
          <Field label={t("fuel.depot.capacity")}>
            <input
              required
              type="number"
              min="1"
...
function ReceiveDialog({
  ...
  return (
    <Dialog title={`${t("fuel.depot.receive")} · ${depot.name}`} onCancel={onCancel}>
      <form onSubmit={submit}>
        <div className="grid gap-4">
          <Field label={t("fuel.depot.which_shipment")}>
            <select
              className={inputClass}
              value={shipmentId}
          ...
          {tanks.length === 0 ? (
            <p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-800">
              {t("fuel.depot.no_tanks")} — {shipment?.fuel_label || shipment?.fuel_type}
            </p>
          ) : (
            <Field label={t("fuel.depot.receive_into")}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label={t("fuel.depot.measured")}>
              <input
                required
                type="number"
                min="1"
            ...
            <Field label={t("fuel.depot.manifest")}>
              <input
                type="number"
                min="0"
                className={inputClass}
          ...
          <p className="text-xs text-slate-400">{t("fuel.depot.manifest_note")}</p>
        </div>
