...
export default function LandingPage(){const {t}=useI18n();return <div className="gp-landing" id="top">
  <header className="gp-nav"><a href="#top" className="gp-brand"><img src="/brand.webp" alt=""/><span>Gerege ERP</span></a><nav><a href="#features">{t("website.menu.features")}</a><a href="#trust">{t("website.menu.trust")}</a><a href="#technology">{t("website.menu.technology")}</a></nav><div className="gp-actions"><LanguageSwitcher variant="dark"/><Link href="/login" className="gp-gold">{t("website.action.sign_in")}</Link></div></header>
  <main>
    <section className="gp-hero"><div className="gp-pattern"/><div className="gp-hero__inner"><div className="gp-copy"><span className="gp-eyebrow"><i/> ERP · eID · SSO</span><h1>{t("website.view.hero_title_lead")} <em>{t("website.view.hero_title_highlight")}</em> {t("website.view.hero_title_tail")}</h1><p>{t("website.view.hero_lede")}</p><div className="gp-cta"><a href="#eid-login" className="gp-gold gp-gold--large">{t("website.action.eid_sign_in")} <ArrowRight/></a><a href="#features" className="gp-outline">{t("website.action.see_features")}</a></div><div className="gp-stats"><span><b>eID</b>{t("website.stat.eid")}</span><span><b>OAuth2 · OIDC</b>{t("website.stat.standards")}</span><span><b>SSO</b>{t("website.stat.sso")}</span></div></div><div id="eid-login" className="gp-login-slot"><EIDLogin compact/></div></div></section>
    ...
    <section className="gp-tech" id="technology"><div><Layers/><h3>Gerege ERP</h3><p>{t("website.tech.erp_body")}</p></div><ArrowRight/><div><Fingerprint/><h3>eID Mongolia</h3><p>{t("website.tech.eid_body")}</p></div><ArrowRight/><div><KeyRound/><h3>OIDC / SSO</h3><p>{t("website.tech.sso_body")}</p></div></section>
  </main><footer className="gp-footer"><span>© 2026 Gerege Systems · Gerege ERP</span><span>{t("website.message.footer_note")}</span></footer>
</div>}
