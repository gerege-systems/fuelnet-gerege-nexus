...
export const api = {
  ...
  // Өртөө — the channel between installations (Settings → Өртөө).
  //
  // Platform routes rather than an app's: a link established by an
  // administrator has to outlive any app being uninstalled, because the tasks
  // in flight over it do not stop existing because somebody removed the board
  // they are shown on.
  getUrtuuPeers: () =>
    fetcher<{
      peers: UrtuuPeer[];
  ...
  inviteUrtuuPeer: (name: string) =>
    fetcher<{ id: string; invite_code: string; expires_in_hours: number }>("/urtuu/peers/invite", {
      method: "POST",
      body: JSON.stringify({ name }),
    }),

  joinUrtuuParent: (input: { invite_code: string; base_url: string; name: string }) =>
    fetcher<{ id: string; parent_installation_id: string }>("/urtuu/peers", {
      method: "POST",
      body: JSON.stringify(input),
    }),

  confirmUrtuuPeer: (id: string) =>
    fetcher<{ id: string }>(`/urtuu/peers/${id}/confirm`, { method: "POST" }),

  revokeUrtuuPeer: (id: string) =>
    fetcher<{ id: string }>(`/urtuu/peers/${id}/revoke`, { method: "POST" }),

  /** Replaces the whole set: what is sent is what the link may carry. */
  setUrtuuPeerCodes: (id: string, codes: string[]) =>
    fetcher<{ peer_id: string; codes: number }>(`/urtuu/peers/${id}/codes`, {
      method: "PUT",
      body: JSON.stringify({ codes }),
    }),

  getUrtuuCodes: () =>
    fetcher<{ codes: UrtuuCode[]; ring_configured: boolean }>("/urtuu/codes"),
  ...
  ) => fetcher<{ id: string }>(`/urtuu/codes/${id}`, {
    method: "PUT",
    body: JSON.stringify(input),
  }),
