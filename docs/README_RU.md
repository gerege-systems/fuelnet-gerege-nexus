...
- **XYP — государственная система обмена данными**
  (`internal/tenant/identity/gerege/xyp.go`): регистрация граждан (`WS100101`) и проверка
...
- **Встроенный провайдер OAuth2 / OIDC**
  ...
  client credentials.
- **Подтверждение адреса электронной почты** (`internal/tenant/emailverify`) — единый
...
- **AI-ассистент** (`internal/tenant/ai/copilot.go`) — диалог с классификацией
  намерений, подключённый к актуальным данным арендатора.
- **Прогноз запасов** (`internal/tenant/ai/handlers.go`) — делегируется
...
Большинство приложений ниже — модули дистрибутивов: они поставляют свой код и
миграции через `pkg/nexus` и не хранятся в репозитории ядра.
