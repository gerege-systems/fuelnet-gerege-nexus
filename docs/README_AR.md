...
- **XYP — تبادل معلومات الدولة** (`internal/tenant/identity/gerege/xyp.go`): السجل المدني
...
- **تأكيد البريد الإلكتروني** (`internal/tenant/emailverify`) — مسار موحَّد لإثبات ملكية
...
- **المساعد الذكي** (`internal/tenant/ai/copilot.go`) — محادثة مُصنَّفة حسب النية
  وموصولة ببيانات المستأجر الحية.
- **واجهة توقع المخزون** (`internal/tenant/ai/handlers.go`) — تفوض إلى قدرة
...
معظم التطبيقات أدناه وحدات توزيع تحمل شيفرتها وترحيلاتها عبر `pkg/nexus`،
ولا توجد داخل مستودع النواة.
