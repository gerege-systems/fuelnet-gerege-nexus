# FuelNet — Монгол Улсын шатахууны урсгал, эрэлт нийлүүлэлтийн нэгдсэн платформ

**Дэд төсөл:** `fuelnet-dgov-mn` → `modules/fuelnet/*`
...
FuelNet нь дараах **дөрвөн** зүйлийг нэг системд нэгтгэнэ:
...
**Дүгнэлт:** хязгаарлалтыг цуцалсан ч бүтцийн эмзэг байдал арилаагүй. Дараагийн давалгаа ирэхэд гар аргаар биш, **систем бэлэн** байх ёстой. FuelNet-ийг "хямралын үед асдаг, тайван үед хяналтын систем болж ажилладаг" гэсэн хоёр горимтой зохионо.
...
```
[1] Гэрээ/захиалга ──▶ [2] Ачилт (эх орон) ──▶ [3] Хил/Гааль ──▶ [4] ★Чанарын шинжилгээ
        │                                                                │
        ▼                                                                ▼
[5] Нөөцийн сав/терминал ──▶ [6] ★Улсын стратегийн нөөц      [7] ★Хэмжил зүй (15°C залруулга)
        │
        ▼
[8] Тээврийн даалгавар ──▶ [9] ★GPS+лац мөрдлөг ──▶ [10] ШТС хүлээн авалт (ATG)
        │
        ▼
[11] ШТС-ын сав ──▶ [12] Түгээгүүр/хошуу (тоолуур) ──▶ [13] ★Ээлжийн заалт
        │
        ▼
[14] ВАУЧЕР ──▶ [15] Хэрэглэгч ──▶ [16] ★e-Barimt/НӨАТ ──▶ [17] ★Нөхцөлдүүлэлт (reconciliation)
```
...
| 14 | Ваучер | Эрх, лимит, нөөцлөлт, хугацаа, redeem, цуцлалт | FuelNet цөм |
...
| 17 | ★Нөхцөлдүүлэлт | Импорт − нөөц − түгээлт = 0 ± зөвшөөрөгдөх алдаа | FuelNet insight |
...
```
1. Тээврийн даалгавар хаагдана
   └─ ШТС-ын ATG савны түвшний өсөлтийг мэдээлнэ
   └─ Хүлээн авсан литр ≈ ачсан литр (зөвшөөрөгдөх зөрүү ±0.3%)
        │
2. `station.replenishment.confirmed` үйл явдал үүснэ
        │
3. ХУВААРИЛАХ ХӨДӨЛГҮҮР (allocation engine) тооцоолно:
        боломжит_литр = хүлээн_авсан
                      − аль_хэдийн_нөөцлөгдсөн_ваучерийн_литр
                      − техникийн_үлдэгдэл (dead stock)
                      − тэргүүлэх_ангиллын_нөөц (%)
        │
4. Слот болгон хуваана:
        ваучерийн_литр = лимит_дүн (₮50,000) ÷ тухайн_үеийн_үнэ
        слотын_тоо = боломжит_литр ÷ ваучерийн_литр
        │
5. Эрхтэй хэрэглэгчдэд ЭРЭМБЭЛЖ санал болгоно (доорх 4.4)
        │
6. Хэрэглэгч 15 мин дотор баталгаажуулна
        ├─ Тийм → ваучер `reserved`, QR үүснэ, ирэх цонх (time window) оноогдоно
        └─ Үгүй → слот пул руу буцна, дараагийн хэрэглэгчид очно
        │
7. ШТС дээр redeem → түгээгүүр литрийг буцаана → `dispensed`
        │
8. Ашиглагдаагүй ваучер хугацаа дуусмагц автоматаар пул руу буцна
```
...
```
DRAFT ──▶ OFFERED ──(15 мин)──▶ RESERVED ──▶ PRESENTED ──▶ DISPENSED ──▶ SETTLED
             │                      │             │
             └──▶ LAPSED            └──▶ EXPIRED  └──▶ VOIDED (ШТС-ын доголдол)
                     │                      │             │
                     └──────────────────────┴─────────────┴──▶ пул руу буцна
```
...
**Офлайн горим:** ШТС-ын терминал сервертэй холбогдож чадахгүй үед FuelNet-ийн нийтийн түлхүүрээр гарын үсгийг **локал** шалгана. Ашигласан ваучерийн ID-г локал bloom filter + жагсаалтад хадгална (давхар ашиглалтаас сэргийлнэ), холбогдмогц sync хийнэ.
...
| Одоо байгаа (Business) | FuelNet-д хэрхэн үйлчлэх |
|---|---|
...
```
fuelnet-dgov-mn/
├── main.go                    ← modules/fuelnet/* нэмж бүртгэнэ
├── catalog/apps.json          ← FuelNet аппуудын каталог (шинэ)
├── catalog_test.go            ← хэвээр, шинэ модулиудыг нэмнэ
├── db/fuelnet/                ← миграц (goose_db_version_fuelnet)
└── modules/
...
edge/                          ← ★ирмэгийн агент (тусдаа binary, ШТС дээр ажиллана)
    ├── cmd/fuelnet-edge/       локал SQLite, офлайн дараалал, sync
...
```
                     ┌─────────────────────────────────────┐
                     │  Хэрэглэгчийн апп (iOS/Android/PWA) │
                     └──────────────┬──────────────────────┘
                                    │ HTTPS
              ┌─────────────────────▼─────────────────────┐
              │  Edge: CDN + WAF + rate limit + queue     │  ← сандралын хамгаалалт
              └─────────────────────┬─────────────────────┘
                                    │
        ┌───────────────────────────┼───────────────────────────┐
        ▼                           ▼                           ▼
┌───────────────┐         ┌──────────────────┐        ┌──────────────────┐
│ voucher-api   │         │ allocation-worker│        │ ingest-api       │
│ (stateless)   │◀───────▶│  (event-driven)  │◀──────▶│ (ATG/POS/GPS)    │
└───────┬───────┘         └────────┬─────────┘        └────────┬─────────┘
        │                          │                           │
        └──────────┬───────────────┴───────────────┬───────────┘
                   ▼                               ▼
        ┌──────────────────────┐        ┌──────────────────────┐
        │ PostgreSQL (primary) │        │ Redis / NATS JetStream│
        │  + read replicas     │        │  (queue, lock, cache) │
        └──────────┬───────────┘        └──────────────────────┘
                   │
                   ▼
        ┌──────────────────────────────────────────────┐
        │ TimescaleDB — цуваа өгөгдөл (савны түвшин,   │
        │ түгээлт, GPS) · ClickHouse-д ч болно         │
        └──────────────────────────────────────────────┘
                   │
                   ▼
        ┌──────────────────────────────────────────────┐
        │ Гадаад: ХУР · Гааль · e-Barimt · SMS · Банк  │
        │ (бүгд ASYNC, circuit breaker-тэй)            │
        └──────────────────────────────────────────────┘
```
...
3. **Circuit breaker + rate limit.** ХУР унавал FuelNet унахгүй — сүүлд амжилттай шалгасан кэшээр (24 цагийн TTL) үргэлжилнэ.
...
| Төлөв | Ойролцоо эзлэх хувь* | FuelNet-ийн хандлага |
|---|---|---|
...
| C. Түгээгүүр цахим, гэхдээ зөвхөн гараар бичдэг | ~35% | **FuelNet POS суулгана** — гол зорилтот бүлэг |
...
C ба D нь ШТС-ын **талаас илүү хувь** — тэдэнд зориулсан бүтээгдэхүүнгүйгээр улсын хэмжээний хяналт боломжгүй. Тиймээс FuelNet өөрийн POS-той байх ёстой.
...
```
┌──────────────────────────────────────────────────────────────┐
│               FuelNet POS (Go, нэг binary)                   │
│  ээлж · гүйлгээ · ваучер · e-Barimt · үнэ · тайлан           │
└───────────────────────────┬──────────────────────────────────┘
                            │  pump.Driver интерфейс
   ┌────────────────────────┼────────────────────────┐
   ▼                        ▼                        ▼
┌────────────┐   ┌────────────────────┐   ┌────────────────────┐
│ IFSF драйв.│   │ Хувийн протоколууд │   │ Импульс/тоолуурын  │
│ LON / TCP  │   │ (brand drivers)    │   │ драйвер (D бүлэг)  │
└────────────┘   └────────────────────┘   └────────────────────┘
       │                    │                        │
       ▼                    ▼                        ▼
   Gilbarco PSS        Wayne DART              Импульс тоолуур
   Doms PSS 5000       Tokheim UDC/TQC         (RS-485 / GPIO)
   Scheidt&Bachmann    Gilbarco Two-Wire/CL    Optical counter
                       Tatsuno PDE             Manual entry UI
                       Censtar / Sanki
                       Adast · Petrotec HDX
                       Nuovo Pignone
...
**Хэрэглэгддэг физик давхарга:** RS-485 / RS-422 / RS-232 / Current loop / LON / Ethernet. FuelNet-ийн **ирмэгийн төхөөрөмж** (edge box — жижиг Linux SBC) нь эдгээрийг USB-serial болон LON интерфейсээр авч, MQTT/gRPC-ээр сервер рүү илгээнэ.
...
> Хямрал өнгөрнө. Хяналтын систем үлдэнэ. FuelNet-ийг эхнээс нь **байнгын төрийн дэд бүтэц** болгож зохионо.
...
| 6 | POS-ын гэрчилгээжүүлэлтийн журам | Аль POS системүүд стандартад нийцэж байгааг батлах — **зөвхөн FuelNet биш**, өрсөлдөөнийг нээлттэй байлгах | Стандартчиллын газар |

> **№6 чухал:** төрийн хяналтын систем нь нэг нийлүүлэгчид түгжигдэх ёсгүй. FuelNet нь **стандартыг** тодорхойлж, лавлагаа хэрэгжүүлэлт (reference implementation) болно. Бусад POS үйлдвэрлэгч ижил API-д нийцвэл гэрчилгээжинэ. Энэ нь улс төрийн эсэргүүцлийг мэдэгдэхүйц бууруулна.
...
FuelNet хоёр горимтой, шилжилт нь бодлогын нэг тохиргоо:
...
```sql
-- ХУВААРИЛАЛТ
fuelnet_entitlement        (id, citizen_rd_hash, vehicle_id, product, tier,
                            daily_cap_mnt, weekly_cap_mnt, zone_id, status, ...)
fuelnet_voucher            (id ULID, entitlement_id, station_id, product,
...
fuelnet_voucher_event      (id, voucher_id, from_state, to_state, actor,
                            idempotency_key, occurred_at, payload jsonb)
fuelnet_allocation_round   (id, station_id, replenishment_id, available_litres,
                            slot_count, policy_version, created_at)

-- ХАНГАМЖ
fuelnet_import_order       (id, importer_id, supplier, product, tonnes,
                            incoterm, currency, price, eta, status)
fuelnet_customs_entry      (id, import_order_id, declaration_no, hs_code,
                            border_post, cleared_at, duties_mnt, litres_15c)
fuelnet_batch              (id, customs_entry_id, lab_cert_no, octane,
                            density, sulphur_ppm, approved bool)
fuelnet_tank               (id, site_id, site_type, product, capacity_l,
                            dead_stock_l, atg_device_id)
fuelnet_tank_reading       (tank_id, ts, level_mm, volume_l, volume_15c_l,
                            temperature_c, water_mm)          -- hypertable
fuelnet_movement           (id, from_tank, to_tank, batch_id, litres,
                            litres_15c, kind, occurred_at)

-- ЛОГИСТИК
fuelnet_dispatch           (id, truck_id, driver_id, from_depot, to_station,
                            loaded_l, loaded_15c_l, seal_no, departed_at,
                            arrived_at, received_l, variance_pct, status)
fuelnet_gps_track          (dispatch_id, ts, lat, lon, speed, ignition)  -- hypertable

-- ШТС ба POS
fuelnet_station            (id, operator_id, name, lat, lon, zone_id, h3_r7,
                            aimag, soum, status, is_border_zone bool,
                            integration_class char)      -- A/B/C/D (6.1)
fuelnet_edge_device        (id, station_id, serial, pubkey, agent_version,
                            last_seen_at, last_seq bigint, status)
fuelnet_pump               (id, station_id, code, brand, driver, transport,
                            address, sealed_at, seal_no)
fuelnet_nozzle             (id, pump_id, code, product, totalizer_l,
                            totalizer_read_at)
fuelnet_shift              (id, station_id, opened_at, closed_at, staff_id,
                            open_totalizers jsonb, close_totalizers jsonb,
                            cash_declared, cash_expected, variance_mnt)
fuelnet_dispense           (id, station_id, nozzle_id, shift_id, seq bigint,
                            voucher_id NULL, ts, litres, unit_price,
                            amount_mnt, payment_kind, ebarimt_no,
                            prev_hash bytea, hash bytea, device_sig bytea)
fuelnet_pos_payment        (id, dispense_id, kind, amount, ref, settled_at)
fuelnet_price_change       (id, station_id, product, old_price, new_price,
                            effective_at, changed_by, source)  -- үнийн хяналт

-- ТӨРИЙН ХЯНАЛТ
fuelnet_chain_anchor       (station_id, day, first_seq, last_seq, head_hash,
                            anchored_at)                  -- WORM хадгалалт
fuelnet_integrity_alert    (id, station_id, kind, detail jsonb, detected_at,
                            severity, resolved_at)        -- gap, totalizer↓, seal
fuelnet_audit_log          (id, actor, level, action, target, ip, at, payload)

-- ТОХИРУУЛГА
fuelnet_policy             (id, version, effective_from, effective_to,
                            rules jsonb, approved_by, approved_at)
```

`fuelnet_policy.rules` нь бүх лимит, коэффициент, жинг агуулна → **бодлого өөрчлөхөд код байршуулахгүй**. G6 зорилт үүгээр биелнэ.
...
### 11.2 FuelNet POS (ШТС-ын оператор)
...
- [ ] `modules/fuelnet/*` араг яс, миграцын түүх (`goose_db_version_fuelnet`)
...
| R6 | Бодлого улс төржих, лимит байнга солигдох | **Өндөр** | Дунд | Бодлогыг өгөгдөл болгосон (`fuelnet_policy`), 5 минутад өөрчилдөг. Түүхийг хадгална |
...
func main() {
	err := platform.Run(platform.Options{
		Modules: func(p nexus.Platform) {
			...
			// FuelNet — зөвхөн шатахууны сүлжээнд байдаг зүйл
			depot.New(p)
...
`catalog_test.go`-ийн хоёр баталгаа хэвээр үйлчилнэ; FuelNet-ийн модулиуд энэ репод байгаа тул тэдгээрийн хувилбарыг каталогтой тулгах гурав дахь тест нэмнэ.
